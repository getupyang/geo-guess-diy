...
2. 从"我发布的题"中选择图片，添加至集锦（参考微信发图交互）
   ...
   - 最少 1 道题，最多 10 道题；超过 10 道时弹出提示。标题栏**不显示** "已选 N/10" 之类的计数，避免给用户凑满额的压力
   - **空状态**（用户尚未发布任何题目）：显示引导文案"你还没有出过题，先去出一道题吧 →"，点击后**直接跳转至题目上传页**（而非回到首页）
   - **有题目时**：在图片网格末尾始终保留一个"+ 去出题"卡片，点击同样跳转至题目上传页，便于用户一边创建题目一边组建集锦
